...
import numpy as np
from ansys.dynamicreporting.core import ADR, Table, BasicLayout
...
data_table = adr.create_item(Table, name="table1", content=np.array([ics, ips, zet], dtype="|S20"), tags="dp=0 type=hex8")
print(data_table._saved)
data_table.labels_row = ["X", "Sin", "Cos"]
data_table.set_tags(f"dp=dp{str(0)} section=data")

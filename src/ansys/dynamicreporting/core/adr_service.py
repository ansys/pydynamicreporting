...
class Service:
    ...
    def connect(
        ...
        if self._url is not None:  # pragma: no cover
            ...
            return
        self.serverobj = report_remote_server.Server(url=url, username=username, password=password, ansys_version=self._ansys_version)
        try:
    ...
    def start(
        ...
        if self._container:
            ...
            self.serverobj = report_remote_server.Server(
                url=f"http://127.0.0.1:{self._port}", username=username, password=password, ansys_version=self._ansys_version
            )

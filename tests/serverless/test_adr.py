...
@pytest.mark.usefixtures("reset_adr_singleton", "reset_adr_singleton_after_class")
...
class TestADRInitPaths:
    def test_init_simple(self, adr_init):
        ...
        assert isinstance(adr.db_directory, str)
        assert isinstance(adr.ansys_installation, str)

    def test_init_empty_should_fail(self, tmp_path, adr_init):
        # empty db directory fails on check for sqlite file
    ...
    def test_init_new_db_dir(self, tmp_path, adr_init):
        import hashlib
        import uuid

        new_dir = tmp_path / f"auto_{hashlib.md5(uuid.uuid1().bytes).hexdigest()}"
        adr = ADR(
            ansys_installation=adr_init.ansys_installation,
            db_directory=new_dir,
        )
        assert Path(adr.db_directory).is_dir() is True

    def test_init_multiple_databases(self, tmp_path, adr_init):
        # existing db directory
        base_dir = Path(__file__).parent / "test_data"
        ...
        assert adr.db_directory is None

    def test_init_multiple_databases_no_media_dir(self, tmp_path, adr_init):
        # existing db directory
        base_dir = Path(__file__).parent / "test_data"
        source_db = base_dir / "documentation_examples"
        dest_db = base_dir / "dest"
        ...
        with pytest.raises(ImproperlyConfiguredError, match="A media directory must be specified"):
            ADR(
                ansys_installation=adr_init.ansys_installation,
                databases=database_config,
            )

    def test_init_in_memory(self, adr_init):
        adr = ADR(
            ansys_installation=adr_init.ansys_installation,
            in_memory=True,
        )
    ...
    def test_adr_opts_instantiation(self, tmp_path, adr_init):
        base_dir = Path(__file__).parent / "test_data"
        local_db = base_dir / "documentation_examples"
        ...
        opts = {
            "CEI_NEXUS_DEBUG": "0",
            "CEI_NEXUS_SECRET_KEY": get_random_secret_key(),
            "CEI_NEXUS_LOCAL_DB_DIR": str(local_db),
            "CEI_NEXUS_LOCAL_MEDIA_DIR": str(media_dir.parent),
            "CEI_NEXUS_LOCAL_STATIC_DIR": str(static_dir),
        }
        ...
        try:
            ...
            assert isinstance(adr.db_directory, str)
            assert isinstance(adr.media_directory, str)
            assert isinstance(adr.static_directory, str)
        finally:
            # Restore environment
            os.environ.clear()
            os.environ.update(original_environ)

    def test_adr_invalid_ansys_path(self, tmp_path):
        base_dir = Path(__file__).parent / "test_data"
        local_db = base_dir / "documentation_examples"
        with pytest.raises(InvalidAnsysPath):
            ADR(
                ansys_installation=str(tmp_path),
                db_directory=local_db,
            )

    def test_adr_missing_db_config_raises(self, tmp_path, adr_init):
